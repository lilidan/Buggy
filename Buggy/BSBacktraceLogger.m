...
+ (NSArray *)bs_backtraceOfAllThread {
    ...
    NSMutableArray *allThread = [NSMutableArray new];
    for(int i = 0; i < thread_count; i++) {
        [allThread addObject:_bs_backtraceOfThread(threads[i])];
    }
    return [allThread copy];
}

#pragma -mark Get call backtrace of a mach_thread
NSArray *_bs_backtraceOfThread(thread_t thread) {
    ...
    for (int i = 0; i < backtraceLength; ++i) {
        ...
        frame.instructionAddress = [@(backtraceBuffer[i]) stringValue];
        frame.package = [[NSString alloc] initWithCString:symbolicated[i].dli_fname encoding:NSUTF8StringEncoding];
        frame.function = [[NSString alloc] initWithCString:symbolicated[i].dli_sname encoding:NSUTF8StringEncoding];
        frame.instructionAddress = [[NSString alloc] initWithFormat:@"0x%lx",backtraceBuffer[i]];
        frame.symbolAddress = [[NSString alloc] initWithFormat:@"0x%p",symbolicated[i].dli_saddr];
        frame.imageAddress = [[NSString alloc] initWithFormat:@"0x%p",symbolicated[i].dli_fbase];
